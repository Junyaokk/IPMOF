...
First, we create an instance to load and preprocess the data.

- *retailer_id*: the unique identifier for the retailer's dataset
- *dara_dir*: the directory path containing the retailer's dataset
- *random_seed*: the seed value used for problem instance generation
- *scenario_unit*: the repelnishment period length (1 week as default)
- *scenario_num*: the number of scenarios within a problem instance


```python
from domain.instance import Instance
instance = Instance(retailer_id='retailer1', 
...
- *need_opt_po*: whether the Gurobi solver should be invoked to optimize the problem (True or False).
- *need_limited_po*: whether the Gurobi solver should be invoked to optimize the problem within time limit (True or False).
- *need_opt_pp*: whether the P-P benchmark should be invoked to optimize the problem (True or False).
- *need_opt_oo*: whether the O-O benchmark should be invoked to optimize the problem by Gurobi (True or False).
